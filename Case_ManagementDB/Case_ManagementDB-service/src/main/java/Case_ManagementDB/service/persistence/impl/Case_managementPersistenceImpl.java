...
public class Case_managementPersistenceImpl
	...
	private FinderPath _finderPathWithPaginationFindByfindByPatient;
	private FinderPath _finderPathWithoutPaginationFindByfindByPatient;
	private FinderPath _finderPathCountByfindByPatient;

	/**
	 * Returns all the case_managements where patientUserId = &#63;.
	 *
	 * @param patientUserId the patient user ID
	 * @return the matching case_managements
	 */
	@Override
	public List<Case_management> findByfindByPatient(long patientUserId) {
		return findByfindByPatient(
			patientUserId, QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	}

	/**
	 * Returns a range of all the case_managements where patientUserId = &#63;.
	 *
	 * <p>
	 * Useful when paginating results. Returns a maximum of <code>end - start</code> instances. <code>start</code> and <code>end</code> are not primary keys, they are indexes in the result set. Thus, <code>0</code> refers to the first result in the set. Setting both <code>start</code> and <code>end</code> to <code>QueryUtil#ALL_POS</code> will return the full result set. If <code>orderByComparator</code> is specified, then the query will include the given ORDER BY logic. If <code>orderByComparator</code> is absent, then the query will include the default ORDER BY logic from <code>Case_managementModelImpl</code>.
	 * </p>
	 ...
	 */
	@Override
	public List<Case_management> findByfindByPatient(
		long patientUserId, int start, int end) {

		return findByfindByPatient(patientUserId, start, end, null);
	}

	/**
	 * Returns an ordered range of all the case_managements where patientUserId = &#63;.
	 *
	 * <p>
	 * Useful when paginating results. Returns a maximum of <code>end - start</code> instances. <code>start</code> and <code>end</code> are not primary keys, they are indexes in the result set. Thus, <code>0</code> refers to the first result in the set. Setting both <code>start</code> and <code>end</code> to <code>QueryUtil#ALL_POS</code> will return the full result set. If <code>orderByComparator</code> is specified, then the query will include the given ORDER BY logic. If <code>orderByComparator</code> is absent, then the query will include the default ORDER BY logic from <code>Case_managementModelImpl</code>.
	 * </p>
	...
	public List<Case_management> findByfindByPatient(
		...
		return findByfindByPatient(
			patientUserId, start, end, orderByComparator, true);
	}
	...
	public List<Case_management> findByfindByPatient(
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				finderPath = _finderPathWithoutPaginationFindByfindByPatient;
				finderArgs = new Object[] {patientUserId};
			}
		}
		else if (useFinderCache) {
			finderPath = _finderPathWithPaginationFindByfindByPatient;
			finderArgs = new Object[] {
				patientUserId, start, end, orderByComparator
			};
		}
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (Case_management case_management : list) {
					if (patientUserId != case_management.getPatientUserId()) {
						list = null;
		...
		if (list == null) {
			...
			sb.append(_FINDER_COLUMN_FINDBYPATIENT_PATIENTUSERID_2);
	...
	/**
	 * Returns the first case_management in the ordered set where patientUserId = &#63;.
	 *
	 * @param patientUserId the patient user ID
	 * @param orderByComparator the comparator to order the set by (optionally <code>null</code>)
	 * @return the first matching case_management
	 ...
	 */
	@Override
	public Case_management findByfindByPatient_First(
			long patientUserId,
			OrderByComparator<Case_management> orderByComparator)
		throws NoSuchCase_managementException {

		Case_management case_management = fetchByfindByPatient_First(
			patientUserId, orderByComparator);
	 ...
	 */
	@Override
	public Case_management fetchByfindByPatient_First(
		long patientUserId,
		OrderByComparator<Case_management> orderByComparator) {

		List<Case_management> list = findByfindByPatient(
			patientUserId, 0, 1, orderByComparator);
	...
	/**
	 * Returns the last case_management in the ordered set where patientUserId = &#63;.
	 *
	 * @param patientUserId the patient user ID
	 * @param orderByComparator the comparator to order the set by (optionally <code>null</code>)
	 * @return the last matching case_management
	 ...
	 */
	@Override
	public Case_management findByfindByPatient_Last(
			long patientUserId,
			OrderByComparator<Case_management> orderByComparator)
		throws NoSuchCase_managementException {

		Case_management case_management = fetchByfindByPatient_Last(
			patientUserId, orderByComparator);
	 ...
	 */
	@Override
	public Case_management fetchByfindByPatient_Last(
		long patientUserId,
		OrderByComparator<Case_management> orderByComparator) {

		int count = countByfindByPatient(patientUserId);
		...
		List<Case_management> list = findByfindByPatient(
			patientUserId, count - 1, count, orderByComparator);
	...
	/**
	 * Returns the case_managements before and after the current case_management in the ordered set where patientUserId = &#63;.
	 *
	 * @param caseId the primary key of the current case_management
	 * @param patientUserId the patient user ID
	 * @param orderByComparator the comparator to order the set by (optionally <code>null</code>)
	 ...
	 */
	@Override
	public Case_management[] findByfindByPatient_PrevAndNext(
			long caseId, long patientUserId,
			OrderByComparator<Case_management> orderByComparator)
		throws NoSuchCase_managementException {
		...
		try {
			...
			array[0] = getByfindByPatient_PrevAndNext(
				session, case_management, patientUserId, orderByComparator,
				true);

			array[1] = case_management;

			array[2] = getByfindByPatient_PrevAndNext(
				session, case_management, patientUserId, orderByComparator,
				false);
	...
	protected Case_management getByfindByPatient_PrevAndNext(
		Session session, Case_management case_management, long patientUserId,
		OrderByComparator<Case_management> orderByComparator,
		boolean previous) {
	...
	/**
	 * Removes all the case_managements where patientUserId = &#63; from the database.
	 *
	 * @param patientUserId the patient user ID
	 */
	@Override
	public void removeByfindByPatient(long patientUserId) {
		for (Case_management case_management :
				findByfindByPatient(
					patientUserId, QueryUtil.ALL_POS, QueryUtil.ALL_POS,
					null)) {
	...
	/**
	 * Returns the number of case_managements where patientUserId = &#63;.
	 *
	 * @param patientUserId the patient user ID
	 * @return the number of matching case_managements
	 */
	@Override
	public int countByfindByPatient(long patientUserId) {
		FinderPath finderPath = _finderPathCountByfindByPatient;

		Object[] finderArgs = new Object[] {patientUserId};
	...
	private static final String _FINDER_COLUMN_FINDBYPATIENT_PATIENTUSERID_2 =
		"case_management.patientUserId = ?";
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByfindByPatient = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByfindByPatient",
			new String[] {
				Long.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			},
			new String[] {"patientUserId"}, true);

		_finderPathWithoutPaginationFindByfindByPatient = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByfindByPatient",
			new String[] {Long.class.getName()}, new String[] {"patientUserId"},
			true);

		_finderPathCountByfindByPatient = new FinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByfindByPatient",
			new String[] {Long.class.getName()}, new String[] {"patientUserId"},
			false);
