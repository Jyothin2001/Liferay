...
<%@ page import="com.ats.common.util.portlet.UserFilterUtil" %>
...
<%
    ...
    for (User u : usersList) {
        if (u.getStatus() == WorkflowConstants.STATUS_APPROVED) {
            activeUsers.add(u);
        }
    }
%>
...
<div class="patient-form-container">
    ...
    <aui:form action="${addPatientURL}" method="post" name="patientForm"  id="patientForm" autocomplete="on">
  ...
  <!-- User Dropdown at Top -->
        ...
        <aui:row>
            <aui:col>
                <aui:select name="patientUserId" label="User" required="true">
                    <aui:option value="">Select User</aui:option>
                    <%
                        for (User u : activeUsers) { // notice variable u, not user
                    %>
                        <aui:option value="<%= u.getUserId() %>">
...
document.addEventListener("DOMContentLoaded", function() {
    const form = document.getElementsByName("patientForm")[0];
