...
export default function AdminPanel({ token, setToken, onLogout }) {
  ...
  const getCountryCode = (code) =>
    currencyCountryMap[code] || code.slice(0, 2).toUpperCase();

  // Login
  const login = async () => {
    try {
      const res = await axios.post('http://192.168.0.102:8000/admin/login', {
        email,
        password,
      });
  ...
  //     await axios.delete(
  //       `http://192.168.0.102:8000/admin/delete_conversion/${id}`,
  //       {
...
const deleteLog = async (id) => {
  ...
  try {
    ...
    await fetchLogs(); // make sure table refreshes after delete
  } catch (err) {
    console.error(err);
    alert("Delete failed: " + (err.response?.data?.detail || err.message));
  }
  ...
  const currentRows = [...sortedLogs].reverse().slice(indexOfFirstRow, indexOfLastRow);
...
//     // Single date match if only one field is filled
// if (filterStartDate && !filterEndDate) {
//   return logDate === filterStartDate; 
// }

//  if (!filterStartDate && filterEndDate) {
//   return logDate === filterEndDate;
//  }

// // Range filtering when both exist
//  if (filterStartDate && logDate < filterStartDate) return false;
//  if (filterEndDate && logDate > filterEndDate) return false;
...
//                     <th>Rate</th>
//                     <th>Action</th>
//                     {/* Sometimes records may be duplicated, test data, or outdated. Admin should remove them. */}
//                   </tr>
//                 </thead>
