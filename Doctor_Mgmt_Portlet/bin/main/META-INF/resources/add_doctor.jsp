...
<portlet:actionURL name="/doctor/add" var="addDoctorFormURL" />

<%
    // Get users list set in MVCRenderCommand
    /* List<User> users = (List<User>) request.getAttribute("usersList"); */
    
    List<User> noRoleActiveUsers = (List<User>) request.getAttribute("noRoleActiveUsers");
%>

<div class="doctor-form-container">
    <aui:form action="${addDoctorFormURL}" method="post" name="addDoctorForm" enctype="multipart/form-data">
        ...
        <aui:select name="doctorUserId" label="Select Doctor User" required="true">
            ...
            <%
        if (noRoleActiveUsers != null && !noRoleActiveUsers.isEmpty()) {
            for (User user : noRoleActiveUsers) {
    %>
    ...
    %>
        <option disabled>No active unassigned users found</option>
    <%
        }
    %>
           <%--  <%
